...
namespace TL
{
    ...
    */
    ...
    //编译期断言，add by plantang
    template<bool> struct CCompileTimeChecker
    {
        CCompileTimeChecker(...){};
    };
    //特化
    template<> struct CCompileTimeChecker<false>{};
...
#define SUPERSUBCLASS(T, U) (taf::TL::Conversion<const U*, const T*>::exists && !(taf::TL::Conversion<const T*, const void*>::sameType))
//相同类型, SUPERSUBCLASS_STRICT判断为false
#define SUPERSUBCLASS_STRICT(T, U) (SUPERSUBCLASS(T, U) && !taf::TL::Conversion<const T, const U>::sameType)

//expr:表达式是否为true，msg:出错输出的信息
#define TAF_STATIC_CHECK(expr, msg) { class ERROR_##msg{}; (void)taf::TL::CCompileTimeChecker<(expr)>(ERROR_##msg()); }
