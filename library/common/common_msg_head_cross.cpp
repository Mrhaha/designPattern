...
int32_t CMsgHeadCross::encode(char* pszOut, int32_t& iOutLength)
{
	if (NULL == pszOut)
	{
		return fail;
	...
	coded_length = CCodeEngine::encode_int8(&ptmp, (uint8_t)m_byCmdType);
	iOutLength += coded_length;
	...
	coded_length = CCodeEngine::encode_int16(&ptmp, (uint16_t)m_nSvrRsp);
	iOutLength += coded_length;

	return success;
}

int32_t CMsgHeadCross::decode(const char* pszIn, int32_t iInLength)
{
	...
	decoded_length = CCodeEngine::decode_int8(&ptmp, (uint8_t*)(&m_byCmdType));
	remained_length -= decoded_length;
	...
	decoded_length = CCodeEngine::decode_int16(&ptmp, (uint16_t*)(&m_nSvrRsp));
	remained_length -= decoded_length;
