...
string TC_MD5::md5file(const string& fileFullName)
{
    ...
    /////////////////////////////////////////////////////////////////////////////////////
    //md5update for file:
    ifstream ifs(fileFullName.c_str());
    if (!ifs)
    {
        return "";
    }
    ifs.seekg(0, ios_base::end);
    uint32_t inputLen = ifs.tellg();
    ...
    if (inputLen >= partLen) 
    {
        ifs.readsome(smallbuff, partLen);
        md5_memcpy((POINTER)&context.buffer[index], (POINTER)smallbuff, partLen);
        md5transform(context.state, context.buffer);

		int len = 0;
        for (i = partLen; i + 63 < inputLen; i += 64)
        {
            len = ifs.readsome(smallbuff, 64);
			if (len < 64) 
			{
				len += ifs.readsome(&smallbuff[len], 64-len);
			}
            md5transform(context.state, (POINTER)smallbuff);
        }
        index = 0;
    }
    inputLen = ifs.readsome(smallbuff, 64);
    md5_memcpy((POINTER)&context.buffer[index], (POINTER)smallbuff, inputLen);
    ifs.close();
    /////////////////////////////////////////////////////////////////////////////////////
...
void TC_MD5::md5update (MD5_CTX *context, unsigned char *input,unsigned int inputLen)
{
    unsigned int i, index, partLen;
    index = (unsigned int)((context->count[0] >> 3) & 0x3F);   /* Compute number of bytes mod 64 */
...
void TC_MD5::md5final (unsigned char digest[16], MD5_CTX *context)
{
...
void TC_MD5::md5transform (UINT4 state[4],unsigned char block[64])
{
...
string TC_MD5::md5bin(std::string_view vsString)
{
    unsigned char sOutBuffer[16];

    MD5_CTX context;
    md5init (&context);
    md5update (&context, (unsigned char*)vsString.data(), vsString.length());
    md5final (sOutBuffer, &context);
...
string TC_MD5::md5str(std::string_view vsString)
{
    string s = md5bin(vsString);
    return bin2str((const void *)s.data(), s.length(), "");
}
