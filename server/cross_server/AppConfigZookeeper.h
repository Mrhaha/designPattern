...
public:
    ...
    typedef std::function<bool(const CAppConfigZookeeper&)> reload_func_type;

    struct config_observer_t
    {
        config_observer_t() : m_stConfigObserverList(), m_iLastUpdateTime(0) {  }

        void UpdateConfig(time_t iLoadTime, const CAppConfigZookeeper& stConfigZookeeper)
        {
...
public:
    typedef std::map<std::string, std::pair<std::string, CAppConfigZookeeper*>> config_map_type;
    ...
    struct config_complete_pool_t
    {
        ...
        void AddConfigFile(const std::string& strFileName, CAppConfigZookeeper* pZookeeperConfig)
        {
...
private:
    CZookeeperInfo                          m_stZookeeperInfo;
