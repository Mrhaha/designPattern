...
const char* CRecordBanquetIDTable::s_strBanquetID = "banquet_id";

CRecordBanquetIDTable::~CRecordBanquetIDTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordBanquetIDTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO banquet_id_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordBanquetIDTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE banquet_id_t SET %s WHERE group_id=%d;", strSet.c_str(), m_iGroupID);
    return szBuffer;
}
std::string
CRecordBanquetIDTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM banquet_id_t WHERE group_id=%d;", m_iGroupID);
    return szBuffer;
}

int
CRecordBanquetIDTable::GetGroupID()
{
    return m_iGroupID;
}

std::vector<TBanquetIDTableValueType>
CRecordBanquetIDTable::SelectDataFromSQL(int iGroupID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM banquet_id_t WHERE group_id=%d;" , iGroupID);
    std::vector<TBanquetIDTableValueType> stRet;
    TBanquetIDTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_ulBanquetID = (uint64_t)strtoull(stData[i]["banquet_id"].c_str(), nullptr, 10);
