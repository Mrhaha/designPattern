...
const char* CRecordGuildIDTable::s_strGuildID = "guild_id";

CRecordGuildIDTable::~CRecordGuildIDTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordGuildIDTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO guild_id_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordGuildIDTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE guild_id_t SET %s WHERE group_id=%d;", strSet.c_str(), m_iGroupID);
    return szBuffer;
}
std::string
CRecordGuildIDTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM guild_id_t WHERE group_id=%d;", m_iGroupID);
    return szBuffer;
}

int
CRecordGuildIDTable::GetGroupID()
{
    return m_iGroupID;
}

std::vector<TGuildIDTableValueType>
CRecordGuildIDTable::SelectDataFromSQL(int iGroupID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM guild_id_t WHERE group_id=%d;" , iGroupID);
    std::vector<TGuildIDTableValueType> stRet;
    TGuildIDTableValueType stValue;
