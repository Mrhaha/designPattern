...
const char* CRecordGuildMemberTable::s_strGuildID = "guild_id";
const char* CRecordGuildMemberTable::s_strPost = "post";
const char* CRecordGuildMemberTable::s_strContributionTotal = "contribution_total";
const char* CRecordGuildMemberTable::s_strJoinTime = "join_time";
const char* CRecordGuildMemberTable::s_strGuildWarHurt = "guild_war_hurt";

CRecordGuildMemberTable::~CRecordGuildMemberTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordGuildMemberTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
...
std::string
CRecordGuildMemberTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE guild_member_t SET %s WHERE uid=%d AND group_id=%d;", strSet.c_str(), m_iUid, m_iGroupID);
    return szBuffer;
}
std::string
CRecordGuildMemberTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM guild_member_t WHERE uid=%d AND group_id=%d;", m_iUid, m_iGroupID);
    return szBuffer;
}

int
CRecordGuildMemberTable::GetUid()
{
    return m_iUid;
}

int
CRecordGuildMemberTable::GetGroupID()
{
    return m_iGroupID;
}

std::vector<TGuildMemberTableValueType>
CRecordGuildMemberTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM guild_member_t WHERE uid=%d AND group_id=%d;" , iUin, iGroupID);
    std::vector<TGuildMemberTableValueType> stRet;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_cPost = (char)::atoi(stData[i]["post"].c_str());
            stValue.m_iContributionTotal = ::atoi(stData[i]["contribution_total"].c_str());
            stValue.m_iJoinTime = ::atoi(stData[i]["join_time"].c_str());
            stValue.m_ulGuildWarHurt = (uint64_t)strtoull(stData[i]["guild_war_hurt"].c_str(), nullptr, 10);
...
std::vector<TGuildMemberTableValueType>
CRecordGuildMemberTable::SelectDataFromSQLWithGuildID(int iGuildID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM guild_member_t WHERE guild_id=%d;" , iGuildID);
    std::vector<TGuildMemberTableValueType> stRet;
    TGuildMemberTableValueType stValue;
