...
CRecordGlobalGuildWarRankTable::MakeInsertSQL()
{
    ...
    strCol += "guild_rank_id" + std::string(",");
    strVal += Int2DBString(m_iGuildRankID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO cross_guild_war_rank_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordGlobalGuildWarRankTable::MakeUpdateSQL()
{
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE cross_guild_war_rank_t SET %s WHERE guild_rank_id=%d AND guild_id=%d;", strSet.c_str(), m_iGuildRankID, m_iGuildID);
    return szBuffer;
}
std::string
...
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM cross_guild_war_rank_t WHERE guild_rank_id=%d AND guild_id=%d;", m_iGuildRankID, m_iGuildID);
    return szBuffer;
}

int
CRecordGlobalGuildWarRankTable::GetGuildRankID()
{
    return m_iGuildRankID;
}
...
CRecordGlobalGuildWarRankTable::SelectDataFromSQL(int iGuildRankID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM cross_guild_war_rank_t WHERE guild_rank_id=%d;" , iGuildRankID);
    ...
    try {
        ...
        {
            stValue.m_iGuildRankID = ::atoi(stData[i]["guild_rank_id"].c_str());
