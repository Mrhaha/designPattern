...
const int CRecordCrossFightInfoTable::TABLE_DIV = 32;

const char* CRecordCrossFightInfoTable::s_strPassed = "passed";
const char* CRecordCrossFightInfoTable::s_strFirstPass = "first_pass";
const char* CRecordCrossFightInfoTable::s_strPlayer = "player";

CRecordCrossFightInfoTable::~CRecordCrossFightInfoTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordCrossFightInfoTable::MakeInsertSQL()
{
    ...
    strCol += "team_id" + std::string(",");
    strVal += Int2DBString(m_iTeamID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO cross_fight_info_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordCrossFightInfoTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE cross_fight_info_t_%d SET %s WHERE uid=%d AND group_id=%d AND team_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iTeamID);
    return szBuffer;
}
std::string
CRecordCrossFightInfoTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM cross_fight_info_t_%d WHERE uid=%d AND group_id=%d AND team_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iTeamID);
    return szBuffer;
}

int
CRecordCrossFightInfoTable::GetUid()
{
    return m_iUid;
}

int
CRecordCrossFightInfoTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordCrossFightInfoTable::GetTeamID()
{
    return m_iTeamID;
}

std::vector<TCrossFightInfoTableValueType>
CRecordCrossFightInfoTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM cross_fight_info_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TCrossFightInfoTableValueType> stRet;
    TCrossFightInfoTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iTeamID = ::atoi(stData[i]["team_id"].c_str());
            stValue.m_iPassed = ::atoi(stData[i]["passed"].c_str());
            stValue.m_iFirstPass = ::atoi(stData[i]["first_pass"].c_str());
            FormatDBBin(stData[i]["player"], stValue.m_stPlayer);
