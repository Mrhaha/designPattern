...
const int CRecordDaySingleRechargeTaskTable::TABLE_DIV = 32;

const char* CRecordDaySingleRechargeTaskTable::s_strRewardCount = "reward_count";
const char* CRecordDaySingleRechargeTaskTable::s_strCanRewardCount = "can_reward_count";

CRecordDaySingleRechargeTaskTable::~CRecordDaySingleRechargeTaskTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordDaySingleRechargeTaskTable::MakeInsertSQL()
{
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO day_single_recharge_task_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordDaySingleRechargeTaskTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE day_single_recharge_task_t_%d SET %s WHERE uid=%d AND group_id=%d AND activity_id=%d AND task_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iActivityID, m_iTaskID);
    return szBuffer;
}
std::string
CRecordDaySingleRechargeTaskTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM day_single_recharge_task_t_%d WHERE uid=%d AND group_id=%d AND activity_id=%d AND task_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iActivityID, m_iTaskID);
    return szBuffer;
}

int
CRecordDaySingleRechargeTaskTable::GetUid()
{
    return m_iUid;
}

int
CRecordDaySingleRechargeTaskTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordDaySingleRechargeTaskTable::GetActivityID()
{
    return m_iActivityID;
}

int
CRecordDaySingleRechargeTaskTable::GetTaskID()
{
    return m_iTaskID;
}

std::vector<TDaySingleRechargeTaskTableValueType>
CRecordDaySingleRechargeTaskTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM day_single_recharge_task_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TDaySingleRechargeTaskTableValueType> stRet;
    TDaySingleRechargeTaskTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iRewardCount = ::atoi(stData[i]["reward_count"].c_str());
            stValue.m_iCanRewardCount = ::atoi(stData[i]["can_reward_count"].c_str());
