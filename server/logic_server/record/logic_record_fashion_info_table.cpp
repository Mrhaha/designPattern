...
const int CRecordFashionInfoTable::TABLE_DIV = 32;

const char* CRecordFashionInfoTable::s_strExpiredTime = "expired_time";

CRecordFashionInfoTable::~CRecordFashionInfoTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordFashionInfoTable::MakeInsertSQL()
{
    ...
    strCol += "fashion_id" + std::string(",");
    strVal += Int2DBString(m_iFashionID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO fashion_info_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordFashionInfoTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE fashion_info_t_%d SET %s WHERE uid=%d AND group_id=%d AND fashion_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iFashionID);
    return szBuffer;
}
std::string
CRecordFashionInfoTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM fashion_info_t_%d WHERE uid=%d AND group_id=%d AND fashion_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iFashionID);
    return szBuffer;
}

int
CRecordFashionInfoTable::GetUid()
{
    return m_iUid;
}

int
CRecordFashionInfoTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordFashionInfoTable::GetFashionID()
{
    return m_iFashionID;
}

std::vector<TFashionInfoTableValueType>
CRecordFashionInfoTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM fashion_info_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TFashionInfoTableValueType> stRet;
    TFashionInfoTableValueType stValue;
