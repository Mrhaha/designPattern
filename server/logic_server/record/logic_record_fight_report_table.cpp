...
const int CRecordFightReportTable::TABLE_DIV = 32;

const char* CRecordFightReportTable::s_strReportContent = "report_content";
const char* CRecordFightReportTable::s_strFightResult = "fight_result";
const char* CRecordFightReportTable::s_strParams = "params";

CRecordFightReportTable::~CRecordFightReportTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordFightReportTable::MakeInsertSQL()
{
    ...
    strCol += "report_type" + std::string(",");
    strVal += Int2DBString(m_iReportType) + ",";
    strCol += "report_id" + std::string(",");
    strVal += Int2DBString(m_iReportID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO fight_report_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordFightReportTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE fight_report_t_%d SET %s WHERE uid=%d AND group_id=%d AND report_type=%d AND report_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iReportType, m_iReportID);
    return szBuffer;
}
std::string
CRecordFightReportTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM fight_report_t_%d WHERE uid=%d AND group_id=%d AND report_type=%d AND report_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iReportType, m_iReportID);
    return szBuffer;
}

int
CRecordFightReportTable::GetUid()
{
    return m_iUid;
}

int
CRecordFightReportTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordFightReportTable::GetReportType()
{
    return m_iReportType;
}

int
CRecordFightReportTable::GetReportID()
{
    return m_iReportID;
}

std::vector<TFightReportTableValueType>
CRecordFightReportTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM fight_report_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TFightReportTableValueType> stRet;
    TFightReportTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iReportType = ::atoi(stData[i]["report_type"].c_str());
            stValue.m_iReportID = ::atoi(stData[i]["report_id"].c_str());
            stValue.m_strReportContent = stData[i]["report_content"];
            stValue.m_cFightResult = (char)::atoi(stData[i]["fight_result"].c_str());
            FormatDBBin(stData[i]["params"], stValue.m_stParams);
