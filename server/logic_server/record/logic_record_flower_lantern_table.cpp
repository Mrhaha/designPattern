...
const int CRecordFlowerLanternTable::TABLE_DIV = 32;

const char* CRecordFlowerLanternTable::s_strLogTime = "log_time";
const char* CRecordFlowerLanternTable::s_strActivityID = "activity_id";
const char* CRecordFlowerLanternTable::s_strEvent = "event";
const char* CRecordFlowerLanternTable::s_strEventValue = "event_value";
const char* CRecordFlowerLanternTable::s_strCandle = "candle";
const char* CRecordFlowerLanternTable::s_strTotalValue = "total_value";
const char* CRecordFlowerLanternTable::s_strTotalCandle = "total_candle";

CRecordFlowerLanternTable::~CRecordFlowerLanternTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordFlowerLanternTable::MakeInsertSQL()
{
    ...
    strCol += "log_index" + std::string(",");
    strVal += Int2DBString(m_iLogIndex) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO flower_lantern_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordFlowerLanternTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE flower_lantern_t_%d SET %s WHERE uid=%d AND group_id=%d AND log_index=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iLogIndex);
    return szBuffer;
}
std::string
CRecordFlowerLanternTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM flower_lantern_t_%d WHERE uid=%d AND group_id=%d AND log_index=%d;", iTableMod, m_iUid, m_iGroupID, m_iLogIndex);
    return szBuffer;
}

int
CRecordFlowerLanternTable::GetUid()
{
    return m_iUid;
}

int
CRecordFlowerLanternTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordFlowerLanternTable::GetLogIndex()
{
    return m_iLogIndex;
}

std::vector<TFlowerLanternTableValueType>
CRecordFlowerLanternTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM flower_lantern_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TFlowerLanternTableValueType> stRet;
    TFlowerLanternTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iLogIndex = ::atoi(stData[i]["log_index"].c_str());
            stValue.m_iLogTime = ::atoi(stData[i]["log_time"].c_str());
            stValue.m_iActivityID = ::atoi(stData[i]["activity_id"].c_str());
            stValue.m_iEvent = ::atoi(stData[i]["event"].c_str());
            stValue.m_iEventValue = ::atoi(stData[i]["event_value"].c_str());
            stValue.m_iCandle = ::atoi(stData[i]["candle"].c_str());
            stValue.m_iTotalValue = ::atoi(stData[i]["total_value"].c_str());
            stValue.m_iTotalCandle = ::atoi(stData[i]["total_candle"].c_str());
