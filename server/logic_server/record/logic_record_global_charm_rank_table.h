...
struct TGlobalCharmRankTable;
struct TGlobalCharmRankTableValueType;

class CRecordGlobalCharmRankTable : public CLogicSQLTableRecordBase
{
public:
    CRecordGlobalCharmRankTable() : m_pCache(nullptr) { }

    virtual ~CRecordGlobalCharmRankTable();
    ...
    static std::vector<TGlobalCharmRankTableValueType> SelectDataFromSQL(int iServerID);
    ...
    TGlobalCharmRankTable* m_pCache;
    ...
    void SetCachePtr(TGlobalCharmRankTable* pCache)
    {
        m_pCache = pCache;
    }
private:
...
struct TGlobalCharmRankTableValueType
{
    TGlobalCharmRankTableValueType()
    : m_iServerID(0)
    , m_iUid(0)
    , m_iGroupID(0)
    , m_ulRankScore(0)
...
struct TGlobalCharmRankTable
{
    TGlobalCharmRankTable() : m_pRecord(nullptr) { }
    ~TGlobalCharmRankTable()
    {
        if(nullptr != m_pRecord) m_pRecord->SetCachePtr(nullptr);
    }

    void InitializeWithoutSQL(const TGlobalCharmRankTableValueType& stData)
    {
        m_stData = stData;
    }
    void Initialize(const TGlobalCharmRankTableValueType& stData)
    {
    ...
    CRecordGlobalCharmRankTable* m_pRecord;
    TGlobalCharmRankTableValueType m_stData;
...
private:
    ...
    CRecordGlobalCharmRankTable* GetRecord()
    {
        if(!m_pRecord)
        {
            m_pRecord = new CRecordGlobalCharmRankTable;
            m_pRecord->SetPrimaryKey(m_stData.m_iServerID, m_stData.m_iUid, m_stData.m_iGroupID);
            m_pRecord->SetCachePtr(this);
            LOGIC_RECORD_CACHE_INS->m_stRecordVec.push_back(std::shared_ptr<CRecordGlobalCharmRankTable>(m_pRecord));
        }
        return m_pRecord;
    }
};
