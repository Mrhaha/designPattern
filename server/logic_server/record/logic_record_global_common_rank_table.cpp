...
const char* CRecordGlobalCommonRankTable::s_strRankScore = "rank_score";

CRecordGlobalCommonRankTable::~CRecordGlobalCommonRankTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordGlobalCommonRankTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO global_common_rank_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordGlobalCommonRankTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE global_common_rank_t SET %s WHERE server_id=%d AND rank_id=%d AND uid=%d AND group_id=%d;", strSet.c_str(), m_iServerID, m_iRankID, m_iUid, m_iGroupID);
    return szBuffer;
}
std::string
CRecordGlobalCommonRankTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM global_common_rank_t WHERE server_id=%d AND rank_id=%d AND uid=%d AND group_id=%d;", m_iServerID, m_iRankID, m_iUid, m_iGroupID);
    return szBuffer;
}

int
CRecordGlobalCommonRankTable::GetServerID()
{
    return m_iServerID;
}

int
CRecordGlobalCommonRankTable::GetRankID()
{
    return m_iRankID;
}

int
CRecordGlobalCommonRankTable::GetUid()
{
    return m_iUid;
}

int
CRecordGlobalCommonRankTable::GetGroupID()
{
    return m_iGroupID;
}

std::vector<TGlobalCommonRankTableValueType>
CRecordGlobalCommonRankTable::SelectDataFromSQL(int iServerID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM global_common_rank_t WHERE server_id=%d;" , iServerID);
    std::vector<TGlobalCommonRankTableValueType> stRet;
    TGlobalCommonRankTableValueType stValue;
