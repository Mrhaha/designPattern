...
const char* CRecordGlobalCountdownRankTable::s_strRankScore = "rank_score";

CRecordGlobalCountdownRankTable::~CRecordGlobalCountdownRankTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordGlobalCountdownRankTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO global_countdown_rank_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordGlobalCountdownRankTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE global_countdown_rank_t SET %s WHERE server_id=%d AND uid=%d AND group_id=%d;", strSet.c_str(), m_iServerID, m_iUid, m_iGroupID);
    return szBuffer;
}
std::string
CRecordGlobalCountdownRankTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM global_countdown_rank_t WHERE server_id=%d AND uid=%d AND group_id=%d;", m_iServerID, m_iUid, m_iGroupID);
    return szBuffer;
}

int
CRecordGlobalCountdownRankTable::GetServerID()
{
    return m_iServerID;
}

int
CRecordGlobalCountdownRankTable::GetUid()
{
    return m_iUid;
}

int
CRecordGlobalCountdownRankTable::GetGroupID()
{
    return m_iGroupID;
}

std::vector<TGlobalCountdownRankTableValueType>
CRecordGlobalCountdownRankTable::SelectDataFromSQL(int iServerID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM global_countdown_rank_t WHERE server_id=%d;" , iServerID);
    std::vector<TGlobalCountdownRankTableValueType> stRet;
    TGlobalCountdownRankTableValueType stValue;
