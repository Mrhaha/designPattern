...
struct TGlobalCountdownRankTable;
struct TGlobalCountdownRankTableValueType;

class CRecordGlobalCountdownRankTable : public CLogicSQLTableRecordBase
{
public:
    CRecordGlobalCountdownRankTable() : m_pCache(nullptr) { }

    virtual ~CRecordGlobalCountdownRankTable();
    ...
    static std::vector<TGlobalCountdownRankTableValueType> SelectDataFromSQL(int iServerID);
    ...
    TGlobalCountdownRankTable* m_pCache;
    ...
    void SetCachePtr(TGlobalCountdownRankTable* pCache)
    {
        m_pCache = pCache;
    }
private:
...
struct TGlobalCountdownRankTableValueType
{
    TGlobalCountdownRankTableValueType()
    : m_iServerID(0)
    , m_iUid(0)
    , m_iGroupID(0)
    , m_ulRankScore(0)
...
struct TGlobalCountdownRankTable
{
    TGlobalCountdownRankTable() : m_pRecord(nullptr) { }
    ~TGlobalCountdownRankTable()
    {
        if(nullptr != m_pRecord) m_pRecord->SetCachePtr(nullptr);
    }

    void InitializeWithoutSQL(const TGlobalCountdownRankTableValueType& stData)
    {
        m_stData = stData;
    }
    void Initialize(const TGlobalCountdownRankTableValueType& stData)
    {
    ...
    CRecordGlobalCountdownRankTable* m_pRecord;
    TGlobalCountdownRankTableValueType m_stData;
...
private:
    ...
    CRecordGlobalCountdownRankTable* GetRecord()
    {
        if(!m_pRecord)
        {
            m_pRecord = new CRecordGlobalCountdownRankTable;
            m_pRecord->SetPrimaryKey(m_stData.m_iServerID, m_stData.m_iUid, m_stData.m_iGroupID);
            m_pRecord->SetCachePtr(this);
            LOGIC_RECORD_CACHE_INS->m_stRecordVec.push_back(std::shared_ptr<CRecordGlobalCountdownRankTable>(m_pRecord));
        }
        return m_pRecord;
    }
};
