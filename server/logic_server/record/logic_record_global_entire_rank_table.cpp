...
const char* CRecordGlobalEntireRankTable::s_strRankScore = "rank_score";

CRecordGlobalEntireRankTable::~CRecordGlobalEntireRankTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordGlobalEntireRankTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO global_entire_rank_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordGlobalEntireRankTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE global_entire_rank_t SET %s WHERE server_id=%d AND uid=%d AND group_id=%d;", strSet.c_str(), m_iServerID, m_iUid, m_iGroupID);
    return szBuffer;
}
std::string
CRecordGlobalEntireRankTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM global_entire_rank_t WHERE server_id=%d AND uid=%d AND group_id=%d;", m_iServerID, m_iUid, m_iGroupID);
    return szBuffer;
}

int
CRecordGlobalEntireRankTable::GetServerID()
{
    return m_iServerID;
}

int
CRecordGlobalEntireRankTable::GetUid()
{
    return m_iUid;
}

int
CRecordGlobalEntireRankTable::GetGroupID()
{
    return m_iGroupID;
}

std::vector<TGlobalEntireRankTableValueType>
CRecordGlobalEntireRankTable::SelectDataFromSQL(int iServerID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM global_entire_rank_t WHERE server_id=%d;" , iServerID);
    std::vector<TGlobalEntireRankTableValueType> stRet;
    TGlobalEntireRankTableValueType stValue;
