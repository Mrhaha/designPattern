...
struct TGlobalEntireRankTable;
struct TGlobalEntireRankTableValueType;

class CRecordGlobalEntireRankTable : public CLogicSQLTableRecordBase
{
public:
    CRecordGlobalEntireRankTable() : m_pCache(nullptr) { }

    virtual ~CRecordGlobalEntireRankTable();
    ...
    static std::vector<TGlobalEntireRankTableValueType> SelectDataFromSQL(int iServerID);
    ...
    TGlobalEntireRankTable* m_pCache;
    ...
    void SetCachePtr(TGlobalEntireRankTable* pCache)
    {
        m_pCache = pCache;
    }
private:
...
struct TGlobalEntireRankTableValueType
{
    TGlobalEntireRankTableValueType()
    : m_iServerID(0)
    , m_iUid(0)
    , m_iGroupID(0)
    , m_ulRankScore(0)
...
struct TGlobalEntireRankTable
{
    TGlobalEntireRankTable() : m_pRecord(nullptr) { }
    ~TGlobalEntireRankTable()
    {
        if(nullptr != m_pRecord) m_pRecord->SetCachePtr(nullptr);
    }

    void InitializeWithoutSQL(const TGlobalEntireRankTableValueType& stData)
    {
        m_stData = stData;
    }
    void Initialize(const TGlobalEntireRankTableValueType& stData)
    {
    ...
    CRecordGlobalEntireRankTable* m_pRecord;
    TGlobalEntireRankTableValueType m_stData;
...
private:
    ...
    CRecordGlobalEntireRankTable* GetRecord()
    {
        if(!m_pRecord)
        {
            m_pRecord = new CRecordGlobalEntireRankTable;
            m_pRecord->SetPrimaryKey(m_stData.m_iServerID, m_stData.m_iUid, m_stData.m_iGroupID);
            m_pRecord->SetCachePtr(this);
            LOGIC_RECORD_CACHE_INS->m_stRecordVec.push_back(std::shared_ptr<CRecordGlobalEntireRankTable>(m_pRecord));
        }
        return m_pRecord;
    }
};
