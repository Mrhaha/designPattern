...
struct TGlobalHomeRankTable;
struct TGlobalHomeRankTableValueType;

class CRecordGlobalHomeRankTable : public CLogicSQLTableRecordBase
{
public:
    CRecordGlobalHomeRankTable() : m_pCache(nullptr) { }

    virtual ~CRecordGlobalHomeRankTable();
    ...
    static std::vector<TGlobalHomeRankTableValueType> SelectDataFromSQL(int iServerID);
    ...
    void SetCachePtr(TGlobalHomeRankTable* pCache)
    {
        m_pCache = pCache;
    }
private:
...
struct TGlobalHomeRankTable
{
    TGlobalHomeRankTable() : m_pRecord(nullptr) { }
    ~TGlobalHomeRankTable()
    {
        if(nullptr != m_pRecord) m_pRecord->SetCachePtr(nullptr);
    }

    void InitializeWithoutSQL(const TGlobalHomeRankTableValueType& stData)
    {
        m_stData = stData;
    }
    void Initialize(const TGlobalHomeRankTableValueType& stData)
    {
    ...
    CRecordGlobalHomeRankTable* m_pRecord;
    TGlobalHomeRankTableValueType m_stData;
...
private:
    ...
    CRecordGlobalHomeRankTable* GetRecord()
    {
        if(!m_pRecord)
        {
            m_pRecord = new CRecordGlobalHomeRankTable;
            m_pRecord->SetPrimaryKey(m_stData.m_iServerID, m_stData.m_iUid, m_stData.m_iGroupID);
            m_pRecord->SetCachePtr(this);
            LOGIC_RECORD_CACHE_INS->m_stRecordVec.push_back(std::shared_ptr<CRecordGlobalHomeRankTable>(m_pRecord));
        }
        return m_pRecord;
    }
};
