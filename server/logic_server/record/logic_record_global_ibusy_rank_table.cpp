...
const char* CRecordGlobalIbusyRankTable::s_strRankScore = "rank_score";

CRecordGlobalIbusyRankTable::~CRecordGlobalIbusyRankTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordGlobalIbusyRankTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO global_ibusy_rank_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordGlobalIbusyRankTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE global_ibusy_rank_t SET %s WHERE server_id=%d AND uid=%d AND group_id=%d;", strSet.c_str(), m_iServerID, m_iUid, m_iGroupID);
    return szBuffer;
}
std::string
CRecordGlobalIbusyRankTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM global_ibusy_rank_t WHERE server_id=%d AND uid=%d AND group_id=%d;", m_iServerID, m_iUid, m_iGroupID);
    return szBuffer;
}

int
CRecordGlobalIbusyRankTable::GetServerID()
{
    return m_iServerID;
}

int
CRecordGlobalIbusyRankTable::GetUid()
{
    return m_iUid;
}

int
CRecordGlobalIbusyRankTable::GetGroupID()
{
    return m_iGroupID;
}

std::vector<TGlobalIbusyRankTableValueType>
CRecordGlobalIbusyRankTable::SelectDataFromSQL(int iServerID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM global_ibusy_rank_t WHERE server_id=%d;" , iServerID);
    std::vector<TGlobalIbusyRankTableValueType> stRet;
    TGlobalIbusyRankTableValueType stValue;
