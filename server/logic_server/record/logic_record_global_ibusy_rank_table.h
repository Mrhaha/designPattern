...
struct TGlobalIbusyRankTable;
struct TGlobalIbusyRankTableValueType;

class CRecordGlobalIbusyRankTable : public CLogicSQLTableRecordBase
{
public:
    CRecordGlobalIbusyRankTable() : m_pCache(nullptr) { }

    virtual ~CRecordGlobalIbusyRankTable();
    ...
    static std::vector<TGlobalIbusyRankTableValueType> SelectDataFromSQL(int iServerID);
    ...
    TGlobalIbusyRankTable* m_pCache;
    ...
    void SetCachePtr(TGlobalIbusyRankTable* pCache)
    {
        m_pCache = pCache;
    }
private:
...
struct TGlobalIbusyRankTableValueType
{
    TGlobalIbusyRankTableValueType()
    : m_iServerID(0)
    , m_iUid(0)
    , m_iGroupID(0)
    , m_ulRankScore(0)
...
struct TGlobalIbusyRankTable
{
    TGlobalIbusyRankTable() : m_pRecord(nullptr) { }
    ~TGlobalIbusyRankTable()
    {
        if(nullptr != m_pRecord) m_pRecord->SetCachePtr(nullptr);
    }

    void InitializeWithoutSQL(const TGlobalIbusyRankTableValueType& stData)
    {
        m_stData = stData;
    }
    void Initialize(const TGlobalIbusyRankTableValueType& stData)
    {
    ...
    CRecordGlobalIbusyRankTable* m_pRecord;
    TGlobalIbusyRankTableValueType m_stData;
...
private:
    ...
    CRecordGlobalIbusyRankTable* GetRecord()
    {
        if(!m_pRecord)
        {
            m_pRecord = new CRecordGlobalIbusyRankTable;
            m_pRecord->SetPrimaryKey(m_stData.m_iServerID, m_stData.m_iUid, m_stData.m_iGroupID);
            m_pRecord->SetCachePtr(this);
            LOGIC_RECORD_CACHE_INS->m_stRecordVec.push_back(std::shared_ptr<CRecordGlobalIbusyRankTable>(m_pRecord));
        }
        return m_pRecord;
    }
};
