...
const char* CRecordGlobalLastPvpRankTable::s_strUid = "uid";
const char* CRecordGlobalLastPvpRankTable::s_strGroupID = "group_id";

CRecordGlobalLastPvpRankTable::~CRecordGlobalLastPvpRankTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordGlobalLastPvpRankTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    strCol += "last_rank" + std::string(",");
    strVal += Int2DBString(m_iLastRank) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO global_last_pvp_rank_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordGlobalLastPvpRankTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE global_last_pvp_rank_t SET %s WHERE server_id=%d AND last_rank=%d;", strSet.c_str(), m_iServerID, m_iLastRank);
    return szBuffer;
}
std::string
CRecordGlobalLastPvpRankTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM global_last_pvp_rank_t WHERE server_id=%d AND last_rank=%d;", m_iServerID, m_iLastRank);
    return szBuffer;
}

int
CRecordGlobalLastPvpRankTable::GetServerID()
{
    return m_iServerID;
}

int
CRecordGlobalLastPvpRankTable::GetLastRank()
{
    return m_iLastRank;
}

std::vector<TGlobalLastPvpRankTableValueType>
CRecordGlobalLastPvpRankTable::SelectDataFromSQL(int iServerID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM global_last_pvp_rank_t WHERE server_id=%d;" , iServerID);
    std::vector<TGlobalLastPvpRankTableValueType> stRet;
    TGlobalLastPvpRankTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iLastRank = ::atoi(stData[i]["last_rank"].c_str());
