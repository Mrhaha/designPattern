...
const char* CRecordGlobalMailTable::s_strBeginTime = "begin_time";
const char* CRecordGlobalMailTable::s_strEndTime = "end_time";
const char* CRecordGlobalMailTable::s_strVipLevel = "vip_level";
const char* CRecordGlobalMailTable::s_strTitle = "title";
const char* CRecordGlobalMailTable::s_strSender = "sender";
const char* CRecordGlobalMailTable::s_strContent = "content";
const char* CRecordGlobalMailTable::s_strAttachment = "attachment";
const char* CRecordGlobalMailTable::s_strChannelIDVec = "channel_id_vec";

CRecordGlobalMailTable::~CRecordGlobalMailTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordGlobalMailTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO global_mail_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordGlobalMailTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE global_mail_t SET %s WHERE server_id=%d AND mail_id=%d;", strSet.c_str(), m_iServerID, m_iMailID);
    return szBuffer;
}
std::string
CRecordGlobalMailTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM global_mail_t WHERE server_id=%d AND mail_id=%d;", m_iServerID, m_iMailID);
    return szBuffer;
}

int
CRecordGlobalMailTable::GetServerID()
{
    return m_iServerID;
}

int
CRecordGlobalMailTable::GetMailID()
{
    return m_iMailID;
}

std::vector<TGlobalMailTableValueType>
CRecordGlobalMailTable::SelectDataFromSQL(int iServerID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM global_mail_t WHERE server_id=%d;" , iServerID);
    std::vector<TGlobalMailTableValueType> stRet;
    TGlobalMailTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iBeginTime = ::atoi(stData[i]["begin_time"].c_str());
            stValue.m_iEndTime = ::atoi(stData[i]["end_time"].c_str());
            stValue.m_iVipLevel = ::atoi(stData[i]["vip_level"].c_str());
            ...
            FormatDBBin(stData[i]["channel_id_vec"], stValue.m_stChannelIDVec);
