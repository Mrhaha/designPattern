...
struct TGlobalMatchRankTable;
struct TGlobalMatchRankTableValueType;

class CRecordGlobalMatchRankTable : public CLogicSQLTableRecordBase
{
public:
    CRecordGlobalMatchRankTable() : m_pCache(nullptr) { }

    virtual ~CRecordGlobalMatchRankTable();
    ...
    static std::vector<TGlobalMatchRankTableValueType> SelectDataFromSQL(int iServerID);
    ...
    TGlobalMatchRankTable* m_pCache;
    ...
    void SetCachePtr(TGlobalMatchRankTable* pCache)
    {
        m_pCache = pCache;
    }
private:
...
struct TGlobalMatchRankTableValueType
{
    TGlobalMatchRankTableValueType()
    : m_iServerID(0)
    , m_iUid(0)
    , m_iGroupID(0)
    , m_ulRankScore(0)
...
struct TGlobalMatchRankTable
{
    TGlobalMatchRankTable() : m_pRecord(nullptr) { }
    ~TGlobalMatchRankTable()
    {
        if(nullptr != m_pRecord) m_pRecord->SetCachePtr(nullptr);
    }

    void InitializeWithoutSQL(const TGlobalMatchRankTableValueType& stData)
    {
        m_stData = stData;
    }
    void Initialize(const TGlobalMatchRankTableValueType& stData)
    {
    ...
    CRecordGlobalMatchRankTable* m_pRecord;
    TGlobalMatchRankTableValueType m_stData;
...
private:
    ...
    CRecordGlobalMatchRankTable* GetRecord()
    {
        if(!m_pRecord)
        {
            m_pRecord = new CRecordGlobalMatchRankTable;
            m_pRecord->SetPrimaryKey(m_stData.m_iServerID, m_stData.m_iUid, m_stData.m_iGroupID);
            m_pRecord->SetCachePtr(this);
            LOGIC_RECORD_CACHE_INS->m_stRecordVec.push_back(std::shared_ptr<CRecordGlobalMatchRankTable>(m_pRecord));
        }
        return m_pRecord;
    }
};
