...
const char* CRecordGlobalMilitaryRankTable::s_strRankScore = "rank_score";

CRecordGlobalMilitaryRankTable::~CRecordGlobalMilitaryRankTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordGlobalMilitaryRankTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO global_military_rank_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordGlobalMilitaryRankTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE global_military_rank_t SET %s WHERE server_id=%d AND uid=%d AND group_id=%d;", strSet.c_str(), m_iServerID, m_iUid, m_iGroupID);
    return szBuffer;
}
std::string
CRecordGlobalMilitaryRankTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM global_military_rank_t WHERE server_id=%d AND uid=%d AND group_id=%d;", m_iServerID, m_iUid, m_iGroupID);
    return szBuffer;
}

int
CRecordGlobalMilitaryRankTable::GetServerID()
{
    return m_iServerID;
}

int
CRecordGlobalMilitaryRankTable::GetUid()
{
    return m_iUid;
}

int
CRecordGlobalMilitaryRankTable::GetGroupID()
{
    return m_iGroupID;
}

std::vector<TGlobalMilitaryRankTableValueType>
CRecordGlobalMilitaryRankTable::SelectDataFromSQL(int iServerID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM global_military_rank_t WHERE server_id=%d;" , iServerID);
    std::vector<TGlobalMilitaryRankTableValueType> stRet;
    TGlobalMilitaryRankTableValueType stValue;
