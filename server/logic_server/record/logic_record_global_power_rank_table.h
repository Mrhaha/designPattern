...
struct TGlobalPowerRankTable;
struct TGlobalPowerRankTableValueType;

class CRecordGlobalPowerRankTable : public CLogicSQLTableRecordBase
{
public:
    CRecordGlobalPowerRankTable() : m_pCache(nullptr) { }

    virtual ~CRecordGlobalPowerRankTable();
    ...
    static std::vector<TGlobalPowerRankTableValueType> SelectDataFromSQL(int iServerID);
    ...
    TGlobalPowerRankTable* m_pCache;
    ...
    void SetCachePtr(TGlobalPowerRankTable* pCache)
    {
        m_pCache = pCache;
    }
private:
...
struct TGlobalPowerRankTableValueType
{
    TGlobalPowerRankTableValueType()
    : m_iServerID(0)
    , m_iUid(0)
    , m_iGroupID(0)
    , m_ulRankScore(0)
...
struct TGlobalPowerRankTable
{
    TGlobalPowerRankTable() : m_pRecord(nullptr) { }
    ~TGlobalPowerRankTable()
    {
        if(nullptr != m_pRecord) m_pRecord->SetCachePtr(nullptr);
    }

    void InitializeWithoutSQL(const TGlobalPowerRankTableValueType& stData)
    {
        m_stData = stData;
    }
    void Initialize(const TGlobalPowerRankTableValueType& stData)
    {
    ...
    CRecordGlobalPowerRankTable* m_pRecord;
    TGlobalPowerRankTableValueType m_stData;
...
private:
    ...
    CRecordGlobalPowerRankTable* GetRecord()
    {
        if(!m_pRecord)
        {
            m_pRecord = new CRecordGlobalPowerRankTable;
            m_pRecord->SetPrimaryKey(m_stData.m_iServerID, m_stData.m_iUid, m_stData.m_iGroupID);
            m_pRecord->SetCachePtr(this);
            LOGIC_RECORD_CACHE_INS->m_stRecordVec.push_back(std::shared_ptr<CRecordGlobalPowerRankTable>(m_pRecord));
        }
        return m_pRecord;
    }
};
