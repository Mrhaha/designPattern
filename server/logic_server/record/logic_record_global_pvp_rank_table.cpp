...
const char* CRecordGlobalPvpRankTable::s_strRankScore = "rank_score";

CRecordGlobalPvpRankTable::~CRecordGlobalPvpRankTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordGlobalPvpRankTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO global_pvp_rank_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordGlobalPvpRankTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE global_pvp_rank_t SET %s WHERE server_id=%d AND uid=%d AND group_id=%d;", strSet.c_str(), m_iServerID, m_iUid, m_iGroupID);
    return szBuffer;
}
std::string
CRecordGlobalPvpRankTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM global_pvp_rank_t WHERE server_id=%d AND uid=%d AND group_id=%d;", m_iServerID, m_iUid, m_iGroupID);
    return szBuffer;
}

int
CRecordGlobalPvpRankTable::GetServerID()
{
    return m_iServerID;
}

int
CRecordGlobalPvpRankTable::GetUid()
{
    return m_iUid;
}

int
CRecordGlobalPvpRankTable::GetGroupID()
{
    return m_iGroupID;
}

std::vector<TGlobalPvpRankTableValueType>
CRecordGlobalPvpRankTable::SelectDataFromSQL(int iServerID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM global_pvp_rank_t WHERE server_id=%d;" , iServerID);
    std::vector<TGlobalPvpRankTableValueType> stRet;
    TGlobalPvpRankTableValueType stValue;
