...
const char* CRecordGlobalTowerRankTable::s_strRankScore = "rank_score";

CRecordGlobalTowerRankTable::~CRecordGlobalTowerRankTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordGlobalTowerRankTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO global_tower_rank_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordGlobalTowerRankTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE global_tower_rank_t SET %s WHERE server_id=%d AND uid=%d AND group_id=%d;", strSet.c_str(), m_iServerID, m_iUid, m_iGroupID);
    return szBuffer;
}
std::string
CRecordGlobalTowerRankTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM global_tower_rank_t WHERE server_id=%d AND uid=%d AND group_id=%d;", m_iServerID, m_iUid, m_iGroupID);
    return szBuffer;
}

int
CRecordGlobalTowerRankTable::GetServerID()
{
    return m_iServerID;
}

int
CRecordGlobalTowerRankTable::GetUid()
{
    return m_iUid;
}

int
CRecordGlobalTowerRankTable::GetGroupID()
{
    return m_iGroupID;
}

std::vector<TGlobalTowerRankTableValueType>
CRecordGlobalTowerRankTable::SelectDataFromSQL(int iServerID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM global_tower_rank_t WHERE server_id=%d;" , iServerID);
    std::vector<TGlobalTowerRankTableValueType> stRet;
    TGlobalTowerRankTableValueType stValue;
