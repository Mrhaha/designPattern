...
const char* CRecordGlobalWorldQuestTable::s_strProgress = "progress";
const char* CRecordGlobalWorldQuestTable::s_strState = "state";
const char* CRecordGlobalWorldQuestTable::s_strOpenTime = "open_time";
const char* CRecordGlobalWorldQuestTable::s_strEndTime = "end_time";
const char* CRecordGlobalWorldQuestTable::s_strAwardInfo = "award_info";

CRecordGlobalWorldQuestTable::~CRecordGlobalWorldQuestTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordGlobalWorldQuestTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO global_world_quest_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordGlobalWorldQuestTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE global_world_quest_t SET %s WHERE server_id=%d AND quest_id=%d;", strSet.c_str(), m_iServerID, m_iQuestID);
    return szBuffer;
}
std::string
CRecordGlobalWorldQuestTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM global_world_quest_t WHERE server_id=%d AND quest_id=%d;", m_iServerID, m_iQuestID);
    return szBuffer;
}

int
CRecordGlobalWorldQuestTable::GetServerID()
{
    return m_iServerID;
}

int
CRecordGlobalWorldQuestTable::GetQuestID()
{
    return m_iQuestID;
}

std::vector<TGlobalWorldQuestTableValueType>
CRecordGlobalWorldQuestTable::SelectDataFromSQL(int iServerID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM global_world_quest_t WHERE server_id=%d;" , iServerID);
    std::vector<TGlobalWorldQuestTableValueType> stRet;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            FormatDBBin(stData[i]["award_info"], stValue.m_stAwardInfo);
