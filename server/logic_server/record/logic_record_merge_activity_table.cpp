...
const int CRecordMergeActivityTable::TABLE_DIV = 32;

const char* CRecordMergeActivityTable::s_strProgress = "progress";
const char* CRecordMergeActivityTable::s_strAward = "award";

CRecordMergeActivityTable::~CRecordMergeActivityTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordMergeActivityTable::MakeInsertSQL()
{
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO merge_activity_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordMergeActivityTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE merge_activity_t_%d SET %s WHERE uid=%d AND group_id=%d AND task_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iTaskID);
    return szBuffer;
}
std::string
CRecordMergeActivityTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM merge_activity_t_%d WHERE uid=%d AND group_id=%d AND task_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iTaskID);
    return szBuffer;
}

int
CRecordMergeActivityTable::GetUid()
{
    return m_iUid;
}

int
CRecordMergeActivityTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordMergeActivityTable::GetTaskID()
{
    return m_iTaskID;
}

std::vector<TMergeActivityTableValueType>
CRecordMergeActivityTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM merge_activity_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TMergeActivityTableValueType> stRet;
    TMergeActivityTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_cAward = (char)::atoi(stData[i]["award"].c_str());
