...
const int CRecordMutiStepTaskTable::TABLE_DIV = 32;

const char* CRecordMutiStepTaskTable::s_strTaskStep = "task_step";
const char* CRecordMutiStepTaskTable::s_strTargetNum = "target_num";

CRecordMutiStepTaskTable::~CRecordMutiStepTaskTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordMutiStepTaskTable::MakeInsertSQL()
{
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO muti_step_task_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordMutiStepTaskTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE muti_step_task_t_%d SET %s WHERE uid=%d AND group_id=%d AND task_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iTaskID);
    return szBuffer;
}
std::string
CRecordMutiStepTaskTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM muti_step_task_t_%d WHERE uid=%d AND group_id=%d AND task_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iTaskID);
    return szBuffer;
}

int
CRecordMutiStepTaskTable::GetUid()
{
    return m_iUid;
}

int
CRecordMutiStepTaskTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordMutiStepTaskTable::GetTaskID()
{
    return m_iTaskID;
}

std::vector<TMutiStepTaskTableValueType>
CRecordMutiStepTaskTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM muti_step_task_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TMutiStepTaskTableValueType> stRet;
    TMutiStepTaskTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iTaskStep = ::atoi(stData[i]["task_step"].c_str());
            stValue.m_iTargetNum = ::atoi(stData[i]["target_num"].c_str());
