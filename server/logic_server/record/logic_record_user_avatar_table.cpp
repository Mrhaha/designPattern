...
const int CRecordUserAvatarTable::TABLE_DIV = 32;

const char* CRecordUserAvatarTable::s_strAvatarType = "avatar_type";
const char* CRecordUserAvatarTable::s_strTime = "time";

CRecordUserAvatarTable::~CRecordUserAvatarTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserAvatarTable::MakeInsertSQL()
{
    ...
    strCol += "avatar_id" + std::string(",");
    strVal += Int2DBString(m_iAvatarID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_avatar_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserAvatarTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_avatar_t_%d SET %s WHERE uid=%d AND group_id=%d AND avatar_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iAvatarID);
    return szBuffer;
}
std::string
CRecordUserAvatarTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_avatar_t_%d WHERE uid=%d AND group_id=%d AND avatar_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iAvatarID);
    return szBuffer;
}

int
CRecordUserAvatarTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserAvatarTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserAvatarTable::GetAvatarID()
{
    return m_iAvatarID;
}

std::vector<TUserAvatarTableValueType>
CRecordUserAvatarTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_avatar_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserAvatarTableValueType> stRet;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iAvatarType = ::atoi(stData[i]["avatar_type"].c_str());
