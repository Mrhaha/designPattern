...
const int CRecordUserBadgeTable::TABLE_DIV = 32;

const char* CRecordUserBadgeTable::s_strIsOwned = "is_owned";
const char* CRecordUserBadgeTable::s_strProgress = "progress";
const char* CRecordUserBadgeTable::s_strOwnedTime = "owned_time";
const char* CRecordUserBadgeTable::s_strProSet = "pro_set";

CRecordUserBadgeTable::~CRecordUserBadgeTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserBadgeTable::MakeInsertSQL()
{
    ...
    strCol += "badge_id" + std::string(",");
    strVal += Int2DBString(m_iBadgeID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_badge_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserBadgeTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_badge_t_%d SET %s WHERE uid=%d AND group_id=%d AND badge_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iBadgeID);
    return szBuffer;
}
std::string
CRecordUserBadgeTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_badge_t_%d WHERE uid=%d AND group_id=%d AND badge_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iBadgeID);
    return szBuffer;
}

int
CRecordUserBadgeTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserBadgeTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserBadgeTable::GetBadgeID()
{
    return m_iBadgeID;
}

std::vector<TUserBadgeTableValueType>
CRecordUserBadgeTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_badge_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserBadgeTableValueType> stRet;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iBadgeID = ::atoi(stData[i]["badge_id"].c_str());
            stValue.m_cIsOwned = (char)::atoi(stData[i]["is_owned"].c_str());
            stValue.m_iProgress = ::atoi(stData[i]["progress"].c_str());
            stValue.m_iOwnedTime = ::atoi(stData[i]["owned_time"].c_str());
            FormatDBBin(stData[i]["pro_set"], stValue.m_stProSet);
