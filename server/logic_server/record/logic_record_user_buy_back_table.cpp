...
const int CRecordUserBuyBackTable::TABLE_DIV = 32;

const char* CRecordUserBuyBackTable::s_strUseBackCount = "use_back_count";
const char* CRecordUserBuyBackTable::s_strBuyBackCount = "buy_back_count";
const char* CRecordUserBuyBackTable::s_strCanBackCount = "can_back_count";

CRecordUserBuyBackTable::~CRecordUserBuyBackTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserBuyBackTable::MakeInsertSQL()
{
    ...
    strCol += "back_type" + std::string(",");
    strVal += Int2DBString(m_iBackType) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_buy_back_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserBuyBackTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_buy_back_t_%d SET %s WHERE uid=%d AND group_id=%d AND back_type=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iBackType);
    return szBuffer;
}
std::string
CRecordUserBuyBackTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_buy_back_t_%d WHERE uid=%d AND group_id=%d AND back_type=%d;", iTableMod, m_iUid, m_iGroupID, m_iBackType);
    return szBuffer;
}

int
CRecordUserBuyBackTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserBuyBackTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserBuyBackTable::GetBackType()
{
    return m_iBackType;
}

std::vector<TUserBuyBackTableValueType>
CRecordUserBuyBackTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_buy_back_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserBuyBackTableValueType> stRet;
    TUserBuyBackTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iBackType = ::atoi(stData[i]["back_type"].c_str());
            stValue.m_iUseBackCount = ::atoi(stData[i]["use_back_count"].c_str());
            stValue.m_iBuyBackCount = ::atoi(stData[i]["buy_back_count"].c_str());
            stValue.m_iCanBackCount = ::atoi(stData[i]["can_back_count"].c_str());
