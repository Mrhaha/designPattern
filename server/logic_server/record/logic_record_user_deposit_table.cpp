...
const int CRecordUserDepositTable::TABLE_DIV = 32;

const char* CRecordUserDepositTable::s_strBuyTimes = "buy_times";
const char* CRecordUserDepositTable::s_strTodayBuyTimes = "today_buy_times";
const char* CRecordUserDepositTable::s_strWeekBuyTimes = "week_buy_times";
const char* CRecordUserDepositTable::s_strDoubleTimes = "double_times";
const char* CRecordUserDepositTable::s_strLastDepositTime = "last_deposit_time";
const char* CRecordUserDepositTable::s_strMonthBuyTimes = "month_buy_times";
const char* CRecordUserDepositTable::s_strDayIntervalBuyTimes = "day_interval_buy_times";

CRecordUserDepositTable::~CRecordUserDepositTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserDepositTable::MakeInsertSQL()
{
    ...
    strCol += "order_id" + std::string(",");
    strVal += Int2DBString(m_iOrderID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_deposit_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserDepositTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_deposit_t_%d SET %s WHERE uid=%d AND group_id=%d AND order_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iOrderID);
    return szBuffer;
}
std::string
CRecordUserDepositTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_deposit_t_%d WHERE uid=%d AND group_id=%d AND order_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iOrderID);
    return szBuffer;
}

int
CRecordUserDepositTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserDepositTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserDepositTable::GetOrderID()
{
    return m_iOrderID;
}

std::vector<TUserDepositTableValueType>
CRecordUserDepositTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_deposit_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserDepositTableValueType> stRet;
    TUserDepositTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iOrderID = ::atoi(stData[i]["order_id"].c_str());
            stValue.m_iBuyTimes = ::atoi(stData[i]["buy_times"].c_str());
            stValue.m_iTodayBuyTimes = ::atoi(stData[i]["today_buy_times"].c_str());
            stValue.m_iWeekBuyTimes = ::atoi(stData[i]["week_buy_times"].c_str());
            stValue.m_iDoubleTimes = ::atoi(stData[i]["double_times"].c_str());
            stValue.m_iLastDepositTime = ::atoi(stData[i]["last_deposit_time"].c_str());
            stValue.m_iMonthBuyTimes = ::atoi(stData[i]["month_buy_times"].c_str());
            stValue.m_iDayIntervalBuyTimes = ::atoi(stData[i]["day_interval_buy_times"].c_str());
