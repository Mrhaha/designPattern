...
const int CRecordUserFriendApplyTable::TABLE_DIV = 32;

const char* CRecordUserFriendApplyTable::s_strApplyTime = "apply_time";

CRecordUserFriendApplyTable::~CRecordUserFriendApplyTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserFriendApplyTable::MakeInsertSQL()
{
    ...
    strCol += "friend_uid" + std::string(",");
    strVal += Int2DBString(m_iFriendUid) + ",";
    strCol += "friend_group_id" + std::string(",");
    strVal += Int2DBString(m_iFriendGroupID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_friend_apply_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserFriendApplyTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_friend_apply_t_%d SET %s WHERE uid=%d AND group_id=%d AND friend_uid=%d AND friend_group_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iFriendUid, m_iFriendGroupID);
    return szBuffer;
}
std::string
CRecordUserFriendApplyTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_friend_apply_t_%d WHERE uid=%d AND group_id=%d AND friend_uid=%d AND friend_group_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iFriendUid, m_iFriendGroupID);
    return szBuffer;
}

int
CRecordUserFriendApplyTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserFriendApplyTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserFriendApplyTable::GetFriendUid()
{
    return m_iFriendUid;
}

int
CRecordUserFriendApplyTable::GetFriendGroupID()
{
    return m_iFriendGroupID;
}

std::vector<TUserFriendApplyTableValueType>
CRecordUserFriendApplyTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_friend_apply_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserFriendApplyTableValueType> stRet;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iFriendUid = ::atoi(stData[i]["friend_uid"].c_str());
            stValue.m_iFriendGroupID = ::atoi(stData[i]["friend_group_id"].c_str());
            stValue.m_iApplyTime = ::atoi(stData[i]["apply_time"].c_str());
