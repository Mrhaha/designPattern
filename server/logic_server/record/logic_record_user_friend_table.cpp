...
const int CRecordUserFriendTable::TABLE_DIV = 32;


CRecordUserFriendTable::~CRecordUserFriendTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserFriendTable::MakeInsertSQL()
{
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_friend_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserFriendTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_friend_t_%d SET %s WHERE uid=%d AND group_id=%d AND friend_uid=%d AND friend_group_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iFriendUid, m_iFriendGroupID);
    return szBuffer;
}
std::string
CRecordUserFriendTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_friend_t_%d WHERE uid=%d AND group_id=%d AND friend_uid=%d AND friend_group_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iFriendUid, m_iFriendGroupID);
    return szBuffer;
}

int
CRecordUserFriendTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserFriendTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserFriendTable::GetFriendUid()
{
    return m_iFriendUid;
}

int
CRecordUserFriendTable::GetFriendGroupID()
{
    return m_iFriendGroupID;
}

std::vector<TUserFriendTableValueType>
CRecordUserFriendTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_friend_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserFriendTableValueType> stRet;
