...
const int CRecordUserHomeExploreTable::TABLE_DIV = 32;

const char* CRecordUserHomeExploreTable::s_strTeam = "team";
const char* CRecordUserHomeExploreTable::s_strEndTime = "end_time";
const char* CRecordUserHomeExploreTable::s_strIsCbt = "is_cbt";
const char* CRecordUserHomeExploreTable::s_strState = "state";

CRecordUserHomeExploreTable::~CRecordUserHomeExploreTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserHomeExploreTable::MakeInsertSQL()
{
    ...
    strCol += "explore_quest_id" + std::string(",");
    strVal += Int2DBString(m_iExploreQuestID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_home_explore_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserHomeExploreTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_home_explore_t_%d SET %s WHERE uid=%d AND group_id=%d AND explore_quest_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iExploreQuestID);
    return szBuffer;
}
std::string
CRecordUserHomeExploreTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_home_explore_t_%d WHERE uid=%d AND group_id=%d AND explore_quest_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iExploreQuestID);
    return szBuffer;
}

int
CRecordUserHomeExploreTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserHomeExploreTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserHomeExploreTable::GetExploreQuestID()
{
    return m_iExploreQuestID;
}

std::vector<TUserHomeExploreTableValueType>
CRecordUserHomeExploreTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_home_explore_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserHomeExploreTableValueType> stRet;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iExploreQuestID = ::atoi(stData[i]["explore_quest_id"].c_str());
            FormatDBBin(stData[i]["team"], stValue.m_stTeam);
            stValue.m_iEndTime = ::atoi(stData[i]["end_time"].c_str());
            stValue.m_cIsCbt = (char)::atoi(stData[i]["is_cbt"].c_str());
