...
const int CRecordUserHomeGardenTable::TABLE_DIV = 32;

const char* CRecordUserHomeGardenTable::s_strSeedID = "seed_id";
const char* CRecordUserHomeGardenTable::s_strSeedStat = "seed_stat";
const char* CRecordUserHomeGardenTable::s_strStatTime = "stat_time";

CRecordUserHomeGardenTable::~CRecordUserHomeGardenTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserHomeGardenTable::MakeInsertSQL()
{
    ...
    strCol += "land_id" + std::string(",");
    strVal += Int2DBString(m_iLandID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_home_garden_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserHomeGardenTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_home_garden_t_%d SET %s WHERE uid=%d AND group_id=%d AND home_id=%d AND land_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iHomeID, m_iLandID);
    return szBuffer;
}
std::string
CRecordUserHomeGardenTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_home_garden_t_%d WHERE uid=%d AND group_id=%d AND home_id=%d AND land_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iHomeID, m_iLandID);
    return szBuffer;
}

int
CRecordUserHomeGardenTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserHomeGardenTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserHomeGardenTable::GetHomeID()
{
    return m_iHomeID;
}

int
CRecordUserHomeGardenTable::GetLandID()
{
    return m_iLandID;
}

std::vector<TUserHomeGardenTableValueType>
CRecordUserHomeGardenTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_home_garden_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserHomeGardenTableValueType> stRet;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iLandID = ::atoi(stData[i]["land_id"].c_str());
            stValue.m_iSeedID = ::atoi(stData[i]["seed_id"].c_str());
            stValue.m_iSeedStat = ::atoi(stData[i]["seed_stat"].c_str());
            stValue.m_iStatTime = ::atoi(stData[i]["stat_time"].c_str());
