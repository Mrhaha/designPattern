...
const int CRecordUserHomeGoodsTable::TABLE_DIV = 32;

const char* CRecordUserHomeGoodsTable::s_strGoodsID = "goods_id";
const char* CRecordUserHomeGoodsTable::s_strPosX = "pos_x";
const char* CRecordUserHomeGoodsTable::s_strPosY = "pos_y";
const char* CRecordUserHomeGoodsTable::s_strStat = "stat";
const char* CRecordUserHomeGoodsTable::s_strQuadrant = "quadrant";
const char* CRecordUserHomeGoodsTable::s_strSeatIndex = "seat_index";

CRecordUserHomeGoodsTable::~CRecordUserHomeGoodsTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserHomeGoodsTable::MakeInsertSQL()
{
    ...
    strCol += "home_id" + std::string(",");
    strVal += Int2DBString(m_iHomeID) + ",";
    strCol += "goods_index" + std::string(",");
    strVal += Int2DBString(m_iGoodsIndex) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_home_goods_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserHomeGoodsTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_home_goods_t_%d SET %s WHERE uid=%d AND group_id=%d AND home_id=%d AND goods_index=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iHomeID, m_iGoodsIndex);
    return szBuffer;
}
std::string
CRecordUserHomeGoodsTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_home_goods_t_%d WHERE uid=%d AND group_id=%d AND home_id=%d AND goods_index=%d;", iTableMod, m_iUid, m_iGroupID, m_iHomeID, m_iGoodsIndex);
    return szBuffer;
}

int
CRecordUserHomeGoodsTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserHomeGoodsTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserHomeGoodsTable::GetHomeID()
{
    return m_iHomeID;
}

int
CRecordUserHomeGoodsTable::GetGoodsIndex()
{
    return m_iGoodsIndex;
}

std::vector<TUserHomeGoodsTableValueType>
CRecordUserHomeGoodsTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_home_goods_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserHomeGoodsTableValueType> stRet;
    TUserHomeGoodsTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iHomeID = ::atoi(stData[i]["home_id"].c_str());
            stValue.m_iGoodsIndex = ::atoi(stData[i]["goods_index"].c_str());
            stValue.m_iGoodsID = ::atoi(stData[i]["goods_id"].c_str());
            stValue.m_iPosX = ::atoi(stData[i]["pos_x"].c_str());
            stValue.m_iPosY = ::atoi(stData[i]["pos_y"].c_str());
            stValue.m_cStat = (char)::atoi(stData[i]["stat"].c_str());
            stValue.m_cQuadrant = (char)::atoi(stData[i]["quadrant"].c_str());
            stValue.m_iSeatIndex = ::atoi(stData[i]["seat_index"].c_str());
