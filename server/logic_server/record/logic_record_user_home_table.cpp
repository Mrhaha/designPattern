...
const int CRecordUserHomeTable::TABLE_DIV = 32;

const char* CRecordUserHomeTable::s_strHomeType = "home_type";
const char* CRecordUserHomeTable::s_strPos = "pos";
const char* CRecordUserHomeTable::s_strPageID = "page_id";
const char* CRecordUserHomeTable::s_strFloorPageID = "floor_page_id";
const char* CRecordUserHomeTable::s_strHeroVec = "hero_vec";

CRecordUserHomeTable::~CRecordUserHomeTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserHomeTable::MakeInsertSQL()
{
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_home_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserHomeTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_home_t_%d SET %s WHERE uid=%d AND group_id=%d AND home_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iHomeID);
    return szBuffer;
}
std::string
CRecordUserHomeTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_home_t_%d WHERE uid=%d AND group_id=%d AND home_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iHomeID);
    return szBuffer;
}

int
CRecordUserHomeTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserHomeTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserHomeTable::GetHomeID()
{
    return m_iHomeID;
}

std::vector<TUserHomeTableValueType>
CRecordUserHomeTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_home_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserHomeTableValueType> stRet;
    TUserHomeTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iHomeType = ::atoi(stData[i]["home_type"].c_str());
            stValue.m_iPos = ::atoi(stData[i]["pos"].c_str());
            stValue.m_iPageID = ::atoi(stData[i]["page_id"].c_str());
            stValue.m_iFloorPageID = ::atoi(stData[i]["floor_page_id"].c_str());
            FormatDBBin(stData[i]["hero_vec"], stValue.m_stHeroVec);
