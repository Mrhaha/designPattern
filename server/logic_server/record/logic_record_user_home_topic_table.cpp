...
const int CRecordUserHomeTopicTable::TABLE_DIV = 32;

const char* CRecordUserHomeTopicTable::s_strHomeTopicInfo = "home_topic_info";

CRecordUserHomeTopicTable::~CRecordUserHomeTopicTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserHomeTopicTable::MakeInsertSQL()
{
    ...
    strCol += "home_topic_id" + std::string(",");
    strVal += Int2DBString(m_iHomeTopicID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_home_topic_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserHomeTopicTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_home_topic_t_%d SET %s WHERE uid=%d AND group_id=%d AND home_topic_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iHomeTopicID);
    return szBuffer;
}
std::string
CRecordUserHomeTopicTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_home_topic_t_%d WHERE uid=%d AND group_id=%d AND home_topic_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iHomeTopicID);
    return szBuffer;
}

int
CRecordUserHomeTopicTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserHomeTopicTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserHomeTopicTable::GetHomeTopicID()
{
    return m_iHomeTopicID;
}

std::vector<TUserHomeTopicTableValueType>
CRecordUserHomeTopicTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_home_topic_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserHomeTopicTableValueType> stRet;
    TUserHomeTopicTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iHomeTopicID = ::atoi(stData[i]["home_topic_id"].c_str());
            FormatDBBin(stData[i]["home_topic_info"], stValue.m_stHomeTopicInfo);
