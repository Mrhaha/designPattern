...
const int CRecordUserHonorTable::TABLE_DIV = 32;

const char* CRecordUserHonorTable::s_strHonorStatus = "honor_status";
const char* CRecordUserHonorTable::s_strExpireTime = "expire_time";
const char* CRecordUserHonorTable::s_strTaskProgress1 = "task_progress_1";
const char* CRecordUserHonorTable::s_strTaskProgress2 = "task_progress_2";
const char* CRecordUserHonorTable::s_strTaskProgress3 = "task_progress_3";
const char* CRecordUserHonorTable::s_strTaskProgress4 = "task_progress_4";
const char* CRecordUserHonorTable::s_strTaskProgress5 = "task_progress_5";

CRecordUserHonorTable::~CRecordUserHonorTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserHonorTable::MakeInsertSQL()
{
    ...
    strCol += "honor_id" + std::string(",");
    strVal += Int2DBString(m_iHonorID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_honor_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserHonorTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_honor_t_%d SET %s WHERE uid=%d AND group_id=%d AND honor_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iHonorID);
    return szBuffer;
}
std::string
CRecordUserHonorTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_honor_t_%d WHERE uid=%d AND group_id=%d AND honor_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iHonorID);
    return szBuffer;
}

int
CRecordUserHonorTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserHonorTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserHonorTable::GetHonorID()
{
    return m_iHonorID;
}

std::vector<TUserHonorTableValueType>
CRecordUserHonorTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_honor_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserHonorTableValueType> stRet;
    TUserHonorTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iHonorID = ::atoi(stData[i]["honor_id"].c_str());
            stValue.m_iHonorStatus = ::atoi(stData[i]["honor_status"].c_str());
            stValue.m_iExpireTime = ::atoi(stData[i]["expire_time"].c_str());
            stValue.m_iTaskProgress1 = ::atoi(stData[i]["task_progress_1"].c_str());
            stValue.m_iTaskProgress2 = ::atoi(stData[i]["task_progress_2"].c_str());
            stValue.m_iTaskProgress3 = ::atoi(stData[i]["task_progress_3"].c_str());
            stValue.m_iTaskProgress4 = ::atoi(stData[i]["task_progress_4"].c_str());
            stValue.m_iTaskProgress5 = ::atoi(stData[i]["task_progress_5"].c_str());
