...
struct TUserIbusyMarketTable;
struct TUserIbusyMarketTableValueType;

class CRecordUserIBusyMarketTable : public CLogicSQLTableRecordBase
{
public:
    CRecordUserIBusyMarketTable() : m_pCache(nullptr) { }

    virtual ~CRecordUserIBusyMarketTable();
    ...
    static std::vector<TUserIbusyMarketTableValueType> SelectDataFromSQL(int iUin, int iGroupID);
    ...
    TUserIbusyMarketTable* m_pCache;
    ...
    void SetCachePtr(TUserIbusyMarketTable* pCache)
    {
        m_pCache = pCache;
    }
private:
...
struct TUserIbusyMarketTableValueType
{
    TUserIbusyMarketTableValueType()
    : m_iUid(0)
    , m_iGroupID(0)
    , m_iCommodityID(0)
    , m_iLastBuyTime(0)
...
struct TUserIbusyMarketTable
{
    TUserIbusyMarketTable() : m_pRecord(nullptr) { }
    ~TUserIbusyMarketTable()
    {
        if(nullptr != m_pRecord) m_pRecord->SetCachePtr(nullptr);
    }

    void InitializeWithoutSQL(const TUserIbusyMarketTableValueType& stData)
    {
        m_stData = stData;
    }
    void Initialize(const TUserIbusyMarketTableValueType& stData)
    {
    ...
    CRecordUserIBusyMarketTable* m_pRecord;
    TUserIbusyMarketTableValueType m_stData;
...
private:
    ...
    CRecordUserIBusyMarketTable* GetRecord()
    {
        if(!m_pRecord)
        {
            m_pRecord = new CRecordUserIBusyMarketTable;
            m_pRecord->SetPrimaryKey(m_stData.m_iUid, m_stData.m_iGroupID, m_stData.m_iCommodityID);
            m_pRecord->SetCachePtr(this);
            LOGIC_RECORD_CACHE_INS->m_stRecordVec.push_back(std::shared_ptr<CRecordUserIBusyMarketTable>(m_pRecord));
        }
        return m_pRecord;
    }
};
