...
const int CRecordUserPayDiamondTable::TABLE_DIV = 32;

const char* CRecordUserPayDiamondTable::s_strPayDiamondLeft = "pay_diamond_left";
const char* CRecordUserPayDiamondTable::s_strPayDiamondConsume = "pay_diamond_consume";

CRecordUserPayDiamondTable::~CRecordUserPayDiamondTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserPayDiamondTable::MakeInsertSQL()
{
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_pay_diamond_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserPayDiamondTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_pay_diamond_t_%d SET %s WHERE uid=%d AND group_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID);
    return szBuffer;
}
std::string
CRecordUserPayDiamondTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_pay_diamond_t_%d WHERE uid=%d AND group_id=%d;", iTableMod, m_iUid, m_iGroupID);
    return szBuffer;
}

int
CRecordUserPayDiamondTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserPayDiamondTable::GetGroupID()
{
    return m_iGroupID;
}

std::vector<TUserPayDiamondTableValueType>
CRecordUserPayDiamondTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_pay_diamond_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserPayDiamondTableValueType> stRet;
    TUserPayDiamondTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            FormatDBBin(stData[i]["pay_diamond_left"], stValue.m_stPayDiamondLeft);
            FormatDBBin(stData[i]["pay_diamond_consume"], stValue.m_stPayDiamondConsume);
