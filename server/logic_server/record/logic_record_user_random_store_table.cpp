...
const int CRecordUserRandomStoreTable::TABLE_DIV = 32;

const char* CRecordUserRandomStoreTable::s_strLastBuyTime = "last_buy_time";
const char* CRecordUserRandomStoreTable::s_strCommodityItemType = "commodity_item_type";
const char* CRecordUserRandomStoreTable::s_strCommodityItemID = "commodity_item_id";
const char* CRecordUserRandomStoreTable::s_strCommodityItemNum = "commodity_item_num";
const char* CRecordUserRandomStoreTable::s_strConsumeItemType = "consume_item_type";
const char* CRecordUserRandomStoreTable::s_strConsumeItemID = "consume_item_id";
const char* CRecordUserRandomStoreTable::s_strConsumeItemNum = "consume_item_num";
const char* CRecordUserRandomStoreTable::s_strDiscount = "discount";

CRecordUserRandomStoreTable::~CRecordUserRandomStoreTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserRandomStoreTable::MakeInsertSQL()
{
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_random_store_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserRandomStoreTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_random_store_t_%d SET %s WHERE uid=%d AND group_id=%d AND store_id=%d AND commodity_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iStoreID, m_iCommodityID);
    return szBuffer;
}
std::string
CRecordUserRandomStoreTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_random_store_t_%d WHERE uid=%d AND group_id=%d AND store_id=%d AND commodity_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iStoreID, m_iCommodityID);
    return szBuffer;
}

int
CRecordUserRandomStoreTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserRandomStoreTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserRandomStoreTable::GetStoreID()
{
    return m_iStoreID;
}

int
CRecordUserRandomStoreTable::GetCommodityID()
{
    return m_iCommodityID;
}

std::vector<TUserRandomStoreTableValueType>
CRecordUserRandomStoreTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_random_store_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserRandomStoreTableValueType> stRet;
    TUserRandomStoreTableValueType stValue;
