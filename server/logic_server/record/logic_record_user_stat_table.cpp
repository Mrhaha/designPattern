...
const int CRecordUserStatTable::TABLE_DIV = 32;

const char* CRecordUserStatTable::s_strPayMoney = "pay_money";

CRecordUserStatTable::~CRecordUserStatTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserStatTable::MakeInsertSQL()
{
    ...
    strCol += "stat_date" + std::string(",");
    strVal += Int2DBString(m_iStatDate) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_stat_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserStatTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_stat_t_%d SET %s WHERE uid=%d AND group_id=%d AND stat_date=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iStatDate);
    return szBuffer;
}
std::string
CRecordUserStatTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_stat_t_%d WHERE uid=%d AND group_id=%d AND stat_date=%d;", iTableMod, m_iUid, m_iGroupID, m_iStatDate);
    return szBuffer;
}

int
CRecordUserStatTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserStatTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserStatTable::GetStatDate()
{
    return m_iStatDate;
}

std::vector<TUserStatTableValueType>
CRecordUserStatTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_stat_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserStatTableValueType> stRet;
    TUserStatTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iStatDate = ::atoi(stData[i]["stat_date"].c_str());
            stValue.m_iPayMoney = ::atoi(stData[i]["pay_money"].c_str());
