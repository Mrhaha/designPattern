...
const int CRecordUserTeamPresetInfoTable::TABLE_DIV = 32;

const char* CRecordUserTeamPresetInfoTable::s_strName = "name";
const char* CRecordUserTeamPresetInfoTable::s_strTeam = "team";

CRecordUserTeamPresetInfoTable::~CRecordUserTeamPresetInfoTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserTeamPresetInfoTable::MakeInsertSQL()
{
    ...
    strCol += "team_index" + std::string(",");
    strVal += Int2DBString(m_iTeamIndex) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_team_preset_info_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserTeamPresetInfoTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_team_preset_info_t_%d SET %s WHERE uid=%d AND group_id=%d AND team_index=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iTeamIndex);
    return szBuffer;
}
std::string
CRecordUserTeamPresetInfoTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_team_preset_info_t_%d WHERE uid=%d AND group_id=%d AND team_index=%d;", iTableMod, m_iUid, m_iGroupID, m_iTeamIndex);
    return szBuffer;
}

int
CRecordUserTeamPresetInfoTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserTeamPresetInfoTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserTeamPresetInfoTable::GetTeamIndex()
{
    return m_iTeamIndex;
}

std::vector<TUserTeamPresetInfoTableValueType>
CRecordUserTeamPresetInfoTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_team_preset_info_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserTeamPresetInfoTableValueType> stRet;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iTeamIndex = ::atoi(stData[i]["team_index"].c_str());
