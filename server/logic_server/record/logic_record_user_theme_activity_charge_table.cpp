...
const int CRecordLogicThemeActivityCharge::TABLE_DIV = 32;

const char* CRecordLogicThemeActivityCharge::s_strTaskType = "task_type";
const char* CRecordLogicThemeActivityCharge::s_strTargetType = "target_type";
const char* CRecordLogicThemeActivityCharge::s_strTargetID = "target_id";
const char* CRecordLogicThemeActivityCharge::s_strTargetAmount = "target_amount";
const char* CRecordLogicThemeActivityCharge::s_strProgress = "progress";
const char* CRecordLogicThemeActivityCharge::s_strState = "state";

CRecordLogicThemeActivityCharge::~CRecordLogicThemeActivityCharge()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordLogicThemeActivityCharge::MakeInsertSQL()
{
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_theme_activity_charge_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordLogicThemeActivityCharge::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_theme_activity_charge_t_%d SET %s WHERE uid=%d AND group_id=%d AND activity_id=%d AND task_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iActivityID, m_iTaskID);
    return szBuffer;
}
std::string
CRecordLogicThemeActivityCharge::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_theme_activity_charge_t_%d WHERE uid=%d AND group_id=%d AND activity_id=%d AND task_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iActivityID, m_iTaskID);
    return szBuffer;
}

int
CRecordLogicThemeActivityCharge::GetUid()
{
    return m_iUid;
}

int
CRecordLogicThemeActivityCharge::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordLogicThemeActivityCharge::GetActivityID()
{
    return m_iActivityID;
}

int
CRecordLogicThemeActivityCharge::GetTaskID()
{
    return m_iTaskID;
}

std::vector<TUserThemeActivityChargeTableValueType>
CRecordLogicThemeActivityCharge::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_theme_activity_charge_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserThemeActivityChargeTableValueType> stRet;
    TUserThemeActivityChargeTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iTargetType = ::atoi(stData[i]["target_type"].c_str());
            stValue.m_iTargetID = ::atoi(stData[i]["target_id"].c_str());
            stValue.m_iTargetAmount = ::atoi(stData[i]["target_amount"].c_str());
