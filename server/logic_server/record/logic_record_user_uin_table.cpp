#include "logic_record_user_uin_table.h"

#include <cassert>


const char* CRecordUserUinTable::s_strAccount = "account";
const char* CRecordUserUinTable::s_strPwd = "pwd";
const char* CRecordUserUinTable::s_strPlatform = "platform";
const char* CRecordUserUinTable::s_strOs = "os";

CRecordUserUinTable::~CRecordUserUinTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserUinTable::MakeInsertSQL()
{
    std::string strCol = "(";
    std::string strVal = "(";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_uin_t %s VALUES %s;" , strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserUinTable::MakeUpdateSQL()
{
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    auto stIntDataITEnd = m_stIntData.end();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_uin_t SET %s WHERE uid=%d;", strSet.c_str(), m_iUid);
    return szBuffer;
}
std::string
CRecordUserUinTable::MakeDeleteSQL()
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_uin_t WHERE uid=%d;", m_iUid);
    return szBuffer;
}

int
CRecordUserUinTable::GetUid()
{
    return m_iUid;
}

std::vector<TUserUinTableValueType>
CRecordUserUinTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_uin_t WHERE uid=%d AND group_id=%d;" , iUin, iGroupID);
    std::vector<TUserUinTableValueType> stRet;
    TUserUinTableValueType stValue;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_strAccount = stData[i]["account"];
            stValue.m_strPwd = stData[i]["pwd"];
            stValue.m_strPlatform = stData[i]["platform"];
            stValue.m_strOs = stData[i]["os"];
