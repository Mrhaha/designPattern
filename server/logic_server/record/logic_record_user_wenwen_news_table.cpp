...
const int CRecordUserWenWenNewsTable::TABLE_DIV = 32;

const char* CRecordUserWenWenNewsTable::s_strPara = "para";
const char* CRecordUserWenWenNewsTable::s_strParaEx = "para_ex";
const char* CRecordUserWenWenNewsTable::s_strTime = "time";

CRecordUserWenWenNewsTable::~CRecordUserWenWenNewsTable()
{
    if (m_pCache != nullptr)
    {
...
std::string
CRecordUserWenWenNewsTable::MakeInsertSQL()
{
    ...
    strCol += "news_type" + std::string(",");
    strVal += Int2DBString(m_iNewsType) + ",";
    strCol += "news_id" + std::string(",");
    strVal += Int2DBString(m_iNewsID) + ",";
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "INSERT INTO user_wenwen_news_t_%d %s VALUES %s;" , iTableMod, strCol.c_str(), strVal.c_str());
    return szBuffer;
}
std::string
CRecordUserWenWenNewsTable::MakeUpdateSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    std::string strSet;
    auto stIntDataITBeg = m_stIntData.begin();
    ...
    char szBuffer[12480];
    snprintf(szBuffer, sizeof(szBuffer), "UPDATE user_wenwen_news_t_%d SET %s WHERE uid=%d AND group_id=%d AND news_type=%d AND news_id=%d;", iTableMod, strSet.c_str(), m_iUid, m_iGroupID, m_iNewsType, m_iNewsID);
    return szBuffer;
}
std::string
CRecordUserWenWenNewsTable::MakeDeleteSQL()
{
    int iTableMod = m_iUid % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "DELETE FROM user_wenwen_news_t_%d WHERE uid=%d AND group_id=%d AND news_type=%d AND news_id=%d;", iTableMod, m_iUid, m_iGroupID, m_iNewsType, m_iNewsID);
    return szBuffer;
}

int
CRecordUserWenWenNewsTable::GetUid()
{
    return m_iUid;
}

int
CRecordUserWenWenNewsTable::GetGroupID()
{
    return m_iGroupID;
}

int
CRecordUserWenWenNewsTable::GetNewsType()
{
    return m_iNewsType;
}

int
CRecordUserWenWenNewsTable::GetNewsID()
{
    return m_iNewsID;
}

std::vector<TUserWenWenNewsTableValueType>
CRecordUserWenWenNewsTable::SelectDataFromSQL(int iUin, int iGroupID)
{
    int iTableMod = iUin % TABLE_DIV;
    char szBuffer[1024];
    snprintf(szBuffer, sizeof(szBuffer), "SELECT * FROM user_wenwen_news_t_%d WHERE uid=%d AND group_id=%d;" , iTableMod, iUin, iGroupID);
    std::vector<TUserWenWenNewsTableValueType> stRet;
    ...
    try {
        ...
        for (size_t i = 0; i < stData.size(); ++i)
        {
            ...
            stValue.m_iNewsType = ::atoi(stData[i]["news_type"].c_str());
            stValue.m_iNewsID = ::atoi(stData[i]["news_id"].c_str());
            stValue.m_iPara = ::atoi(stData[i]["para"].c_str());
            stValue.m_iParaEx = ::atoi(stData[i]["para_ex"].c_str());
            stValue.m_iTime = ::atoi(stData[i]["time"].c_str());
