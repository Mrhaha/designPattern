...
const char* CQueueAppConfigZookeeper::Type2String(int iEventType)
{
...
const char* CQueueAppConfigZookeeper::State2String(int iState)
{
...
const char* CQueueAppConfigZookeeper::Errno2String(int iRet)
{
    return (zerror(iRet));
}

/////////////////////////////////////////////////////////////////////////////////
void CQueueAppConfigZookeeper::ZooHandleEventWatcher(zhandle_t* pZooHandle, int iType, int iState, const char* szPath, void* pContext)
{
    CQueueAppConfigZookeeper* pSelf = static_cast<CQueueAppConfigZookeeper*>(pContext);
    TRACESVR(log_mask_detail, "[CQueueAppConfigZookeeper::%s]ZOOKEEPER|WATCHER:%s|STATE:%s\n", __FUNCTION__, Type2String(iType), State2String(iState));
    ...
    if (iType == ZOO_SESSION_EVENT)
    {
        if (iState == ZOO_CONNECTED_STATE)
        {
            ...
            TRACESVR(log_mask_detail, "[CQueueAppConfigZookeeper::%s]ZOOKEEPER||GOT A NEW SESSION ID:%ld\n", __FUNCTION__, pClientID->client_id);

        }
        else if (iState == ZOO_AUTH_FAILED_STATE)
        {
            TRACESVR(log_mask_system_error, "[CQueueAppConfigZookeeper::%s]ZOOKEEPER|ZOOKEEPER AUTHENTICATION FAILURE\n", __FUNCTION__);

            pSelf->SetZookeeperAbnormal();
        }
        else if (iState == ZOO_EXPIRED_SESSION_STATE)
        {
            TRACESVR(log_mask_system_error, "[CQueueAppConfigZookeeper::%s]ZOOKEEPER|SESSION EXPIRED\n", __FUNCTION__);
...
//配置文件内容状态的回调
void CQueueAppConfigZookeeper::ConfigContentEventWatcher(zhandle_t* pZooHandle, int iType, int iState, const char* szPath, void* pContext)
{
    ...
    */
    ...
    CQueueAppConfigZookeeper::config_map_type::value_type* pValue = (CQueueAppConfigZookeeper::config_map_type::value_type*)pContext;
    CQueueAppConfigZookeeper* pSelf = pValue->second.second;
    if (ZOO_DELETED_EVENT == iType)
    {
        pSelf->m_stConfigCompletePool.DeleteConfigFile(std::string(szPath));
        TRACESVR(log_mask_detail, "[CQueueAppConfigZookeeper::ConfigContentEventWatcher]DELETE CONFIG|CONFIG::%s\n", szPath);
    }
    else
    {
        ...
        if (ZOK != iRet)
        {
            TRACESVR(log_mask_system_error, "[CQueueAppConfigZookeeper::ConfigContentEventComplete]WATCH CONFIG ERROR|CONFIG:%s|ERRCODE:%d|ERRMSG:%s\n",
...
void CQueueAppConfigZookeeper::ConfigContentEventComplete(int iRet, const char* iValue, int iLength, const struct Stat* stStat, const void* pContext)
{
    (void)stStat;
    CQueueAppConfigZookeeper::config_map_type::value_type* pValue = (CQueueAppConfigZookeeper::config_map_type::value_type*)pContext;
    CQueueAppConfigZookeeper* pSelf = pValue->second.second;
    if (ZOK == iRet)
    {
        ...
        TRACESVR(log_mask_system_error, "[CQueueAppConfigZookeeper::ConfigContentEventComplete]CONFIG CONTENT COMPLETE|PATH:%s\n", pValue->first.c_str());
    }
    else
    {
        TRACESVR(log_mask_system_error, "[CQueueAppConfigZookeeper::ConfigContentEventComplete]CONFIG CONTENT COMPLETE ERROR|CONFIG:%s|ERRCODE:%d|ERRMSG:%s\n",
...
/////////////////////////////////////////////////////////////////////////////////
CQueueAppConfigZookeeper::CQueueAppConfigZookeeper()
...
void CQueueAppConfigZookeeper::SetZookeeperInfo(const std::string& host, const std::string& config_path)
{
    m_stZookeeperInfo.m_strHost = host;
    m_stZookeeperInfo.m_strNormalConfigNodePath = config_path;
}

void CQueueAppConfigZookeeper::RegsterObserver(const reload_func_type& stObserver)
{
    m_stConfigObserver.m_stConfigObserverList.push_back(stObserver);
}

void CQueueAppConfigZookeeper::ClearZooKeeperInfo()
{
...
void CQueueAppConfigZookeeper::SetZookeeperAbnormal()
{
    m_enuZookeeperHandleStatus = NEED_DESTORY;
}

bool CQueueAppConfigZookeeper::IsZookeeperNormal() const
{
    return (m_enuZookeeperHandleStatus == NORMAL);
}

int CQueueAppConfigZookeeper::WatchAndGetConfigContent()
{
    int iRet = ZOK;
    for (auto& stConfigKV : m_stConfigCompletePool.m_stConfigFileMap)
    {
        ...
        if (iRet != ZOK)
        {
            TRACESVR(log_mask_system_error, "[CQueueAppConfigZookeeper::WatchAndGetConfigContent]WATCH CONFIG ERROR|CONFIG:%s|ERRCODE:%d|ERRMSG:%s\n",
...
bool CQueueAppConfigZookeeper::InitZookeeper()
{
    ...
    if (ZOK != iRet)
    {
        TRACESVR(log_mask_system_error, "[CQueueAppConfigZookeeper::InitZookeeper]watch and get config content error, path:%s code:%d msg:%s\n",
...
void CQueueAppConfigZookeeper::DestoryZookeeper()
{
    zookeeper_close(m_pZooHandle);
    ClearZooKeeperInfo();
}

int CQueueAppConfigZookeeper::Interest(int& iFd, int& iEvent)
{
    iFd = -1;
    iEvent = 0;
    ...
    if (m_pZooHandle)
    {
        ...
        if (iRet != ZOK)
        {
            TRACESVR(log_mask_system_error, "[CQueueAppConfigZookeeper::Interest]ZOOKEEPER INTEREST ERROR|ERRCODE:%d msg:%s\n",
...
int CQueueAppConfigZookeeper::Process(int iEvent)
{
...
void CQueueAppConfigZookeeper::UpdateConfig()
{
...
bool CQueueAppConfigZookeeper::WaitAllConfigReady(int iTimeout)
{
...
void CQueueAppConfigZookeeper::AddWatchFile(const std::string& strFileName)
{
...
const std::string& CQueueAppConfigZookeeper::GetNormalConfig(const std::string& strFileName) const
{
    const std::string strFullName = m_stZookeeperInfo.m_strNormalConfigNodePath + "/" + strFileName;
